...
tA.append(tuple([[0], [0], [0]]))
RA.append(tuple(np.zeros((3,3))))
# Plotting values for relative scale
tR.append(tuple([[0], [0], [0]]))
RR.append(tuple(np.zeros((3,3))))
i = 1
...
while(i <= 151):
	...
	print(t_currR)
	if GTLoc is not None:
		scale1 = AbsoluteScale(groundTruthValues, i)
		t_currA = t_currA + scale1 * R_currA.dot(t0)
		R_currA = R0.dot(R_currA) 
		#tA.append(tuple(t_currA))
		#RA.append(tuple(R_currA))			
	...
	cv2.circle(canvas, (abs(t_currA[0]) + 200, abs(t_currA[2]) + 200), 2, (0,0,255), -1)
